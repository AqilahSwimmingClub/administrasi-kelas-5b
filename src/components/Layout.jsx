import { LayoutDashboard, Users, CalendarCheck, ClipboardList, UserRound, Settings, LogOut, GraduationCap, Menu, X, Bell, ShieldCheck } from 'lucide-react'
import { useState } from 'react'
const items=[['dashboard','Beranda',LayoutDashboard],['students','Data Siswa',Users],['attendance','Absensi',CalendarCheck],['grades','Penilaian',ClipboardList],['parents','Portal Orang Tua',UserRound],['settings','Pengaturan',Settings]]
export default function Layout({page,setPage,onLogout,settings,children}){
 const [open,setOpen]=useState(false)
 return <div className="app-shell">
  <aside className={open?'sidebar open':'sidebar'}>
   <div className="brand"><ShieldCheck size={36}/><div><b>ADMINISTRASI</b><small>KELAS {settings.className}</small></div><button className="icon mobile-close" onClick={()=>setOpen(false)}><X/></button></div>
