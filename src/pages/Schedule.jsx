...
const empty={date:localDate(),start:'07:00',end:'08:00',title:'',type:'Kegiatan Kelas'}
...
export default function Schedule({data,setData}){
 ...
 const submit=e=>{e.preventDefault();if(!form.title.trim())return;setData(d=>({...d,schedules:[...(d.schedules||[]),{...form,id:`ag${Date.now()}`}],notifications:[notice('Agenda baru',`${form.title.trim()} — ${formatIndonesianDate(form.date)} ${form.start}`),...(d.notifications||[])]}));setForm({...empty,date:form.date})}
